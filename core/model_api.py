...
class ModelAPI:
    """Handles asynchronous interaction with the AI model API."""
    ...
    async def generate_response(self, model: str, prompt: str, temperature: float = None) -> str:
        ...
        try:
            ...
            error_msg = f"Request to model API timed out after {timeout} seconds"
    ...
    async def stream_response(self, model: str, prompt: str, callback: Callable[[str], Awaitable[None]],
        ...
        try:
            ...
            error_msg = f"Streaming request to model API timed out after {timeout} seconds"
            logger.error(error_msg)
            await callback(f"\n\n{error_msg}")
